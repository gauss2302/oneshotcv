...
export const PersonalForm: React.FC = () => {
  const { personalInfo, updatePersonal, selectedTemplate } = useCVStore();
  
  const { register, control, formState: { errors } } = useForm<PersonalFormData>({
  ...
  React.useEffect(() => {
    // We cast watchedValues to PersonalFormData because useWatch returns a Partial or DeepPartial
    // and we know the shape matches. We might want to debounce this in a real app.
    if (watchedValues) {
      updatePersonal(watchedValues as Partial<PersonalFormData>);
    }
  }, [watchedValues, updatePersonal]);
